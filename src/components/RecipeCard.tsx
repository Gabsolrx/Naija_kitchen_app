...
export function RecipeCard({ recipe, featured }: RecipeCardProps) {
  ...
  if (featured) {
    return (
      <Link to={`/recipe/${recipe.id}`} className="block relative w-full h-64 rounded-3xl overflow-hidden shadow-lg active:scale-[0.98] transition-transform">
  ...
  return (
    <Link to={`/recipe/${recipe.id}`} className="flex gap-3 p-3 bg-white rounded-2xl shadow-sm border border-gray-100 active:scale-[0.98] transition-transform">
